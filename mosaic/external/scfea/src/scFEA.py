...
def main(args):
    ...
    BATCH_SIZE = min(geneExpr.shape[0], 8192)  # cap to avoid GPU OOM on large datasets
    ...
    X = geneExprDf.values.astype("float32").T
...
# =============================================================================
    ...
    ckpt_path = res_dir + '/model_weights.pt'
...
#    Dataloader
    ...
    for epoch in range(1):
        ...
        for i, (X, X_scale, _) in enumerate(test_loader):
            ...
            # save data
            bs = out_m_batch.shape[0]
            fluxStatuTest[idx:idx+bs, :] = out_m_batch.detach().cpu().numpy()
            balanceStatus[idx:idx+bs, :] = out_c_batch.detach().cpu().numpy()
            idx += bs
    ...
    if fileName == 'NULL':
        ...
        fileName = res_dir + "/" + test_file[-len(test_file):-4] + ".csv"
    ...
    if balanceName == 'NULL':
        ...
        balanceName = res_dir + "/" + "balance" + ".csv"
