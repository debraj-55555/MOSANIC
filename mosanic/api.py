"""
mosanic/api.py — High-level Python API for MOSANIC.

After training (or loading a checkpoint), users get a MOSANICResult object
that provides clean methods for querying CCC signals.

Usage (programmatic):
    from mosanic import train, load
...
# ---------------------------------------------------------------------------
# MOSANICResult: the main user-facing object
# ---------------------------------------------------------------------------

class MOSANICResult:
    """
    Result object returned after training or loading a MOSANIC model.
    ...
    Attributes:
        model:      Trained MOSANIC model (eval mode)
    ...
    def _compute_intensity_scores(self) -> Dict[Tuple[str, str], float]:
        ...
        This is the primary scoring used in all MOSANIC figures. It naturally
        ...
        Args:
            ...
            scoring:   Scoring method:
                       "intensity" (default): attn × mean(expr_L) × mean(expr_R)
                           — used in all MOSANIC figures. Naturally breaks
    ...
    def lr_activity(
        ...
        Uses raw attention by default (matching all MOSANIC figures).
    ...
    def knockout(self, per_gene: bool = False) -> Dict:
        ...
        """
        from mosanic.analysis.knockout import run_knockout
    ...
    def relays(
        ...
        """
        from mosanic.analysis.relay import detect_relays
    ...
    def evaluate_against(
        ...
        """AUROC of MOSANIC's LR scores against an independent reference catalogue.

        Treats each LR pair in MOSANIC's vocabulary as a binary-classifier
        ...
        if not y_true or sum(y_true) == 0 or sum(y_true) == len(y_true):
            raise ValueError(
                "Cannot evaluate: reference DB has no positives in MOSANIC's "
    ...
    def __repr__(self) -> str:
        return (
            f"MOSANICResult(dataset='{self.dataset}', "
...
def train(
    ...
    """
    Train MOSANIC and return a MOSANICResult for interactive analysis.

    Usage:
        from mosanic import train
        result = train("configs/breast.yaml", device="cuda")
    ...
    trainer = MOSANICTrainer(model=model, data=data, config=cfg, device=dev, dataset=dataset)
    ...
    ckpt_dir = Path(cfg.get("training", {}).get("checkpoint_dir", "mosanic/checkpoints")) / dataset
    ckpt_path = ckpt_dir / "model_best.pt"
    if ckpt_path.exists():
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        model.load_state_dict(ckpt.get("model_state_dict", ckpt))

    return MOSANICResult(
...
def load(
    ...
    """
    Load a trained MOSANIC model and return a MOSANICResult.
    ...
    if not graph_path.exists():
        raise FileNotFoundError(
            ...
            f"Run: mosanic preprocess --config {config_path}"
        )
...
def _find_package_dir() -> Path:
    """Locate the MOSANIC package installation directory."""
...
def _build_config(
    ...
    """
    Auto-generate a complete MOSANIC config from minimal user input.
...
def train_model(
    ...
    """
    Step 3: Train MOSANIC and return result object.
    ...
    Returns:
        MOSANICResult ready for analysis
    """
    import yaml
    with open(config_path) as f:
        cfg = yaml.safe_load(f)
    ...
    result = MOSANICResult(model=model, data=data, meta=meta,
                          config=cfg, dataset=dataset, device=dev)
...
def run_pipeline(
    ...
    """
    Run the full MOSANIC pipeline from a single h5ad file.

    This is the recommended entry point for new users. Handles config
    generation, preprocessing, training, and returns a MOSANICResult.
    ...
    Args:
        ...
        output_dir:     Where to save all outputs (default: ./mosanic_output)
    ...
    Returns:
        MOSANICResult ready for LR/MR queries, spatial maps, knockouts,
    ...
    log.info("MOSANIC Pipeline: %s (%s, %s) → %s",
