"""
mosanic/data/preprocessor.py — Unified preprocessing entry point.
...
Usage:
    python -m mosanic.data.preprocessor --config mosanic/configs/breast_config.yaml \\
...
def _step3_lr_database(cfg):
    ...
    import pandas as pd
    from mosanic.data.lr_database import DatabaseLoader
    from mosanic.data.channel_classifier import classify_dataframe
...
def _run_scfea(cfg, adata_path, scfea_balance_path):
    ...
    Paths: all I/O goes under MOSANIC/data/processed/<dataset>/scfea/.
    ...
    # Locate scFEA directory (shipped with MOSANIC)
    ...
    # All scFEA I/O under processed_dir/scfea/ (relative to MOSANIC root)
...
def _step6_node_features_full(step1, step5, step4_data, cfg):
    ...
    from mosanic.graph.node_features import NodeFeatureBuilder
...
def _step16_spatial_splits(step2, cfg):
    ...
    from mosanic.data.spatial_cv import make_spatial_splits
...
def preprocess_dataset(config_path: str, dataset: str, force: bool = False):
    ...
    log.debug(f"=== MOSANIC preprocessing: {dataset} → {out_dir} ===")
...
def _assemble_and_save(cache: dict, cfg: dict, out_dir: Path):
    from mosanic.graph.assembler import GraphAssembler
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MOSANIC preprocessing pipeline")
