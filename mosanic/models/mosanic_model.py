"""
mosanic/models/mosanic_model.py

MOSANIC: Heterogeneous Graph Transformer for Cell-Cell Communication.
...
class MOSANIC(nn.Module):
    """
    MOSANIC model: HetGTEncoder + ExpressionDecoder.
...
def build_model(cfg: dict, n_expr_genes: int, graph_metadata: dict = None) -> "MOSANIC":
    """
    Instantiate MOSANIC from config dict.
    ...
    Returns:
        MOSANIC model (not yet moved to device)
    """
    edge_type_dims = None
