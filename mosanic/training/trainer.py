...
Key changes vs src4:
  ...
  - Checkpoint dir: mosanic/checkpoints/<dataset>/
...
class MOSANICTrainer:
    """
    Full-graph trainer for MOSANIC (expression prediction primary task).

    Args:
        model:   MOSANIC model
        data:    PyG HeteroData (MOSANIC format, full graph on device)
    ...
    def __init__(
        ...
        # -- Loss --------------------------------------------------------
        self.criterion = MOSANICLoss(
            ccc_weight=0.0,          # no auxiliary tasks in MOSANIC
        ...
        ckpt_dir = Path(train_cfg.get("checkpoint_dir", "mosanic/checkpoints")) / dataset
    ...
    def train(self, num_epochs: int) -> Dict[str, list]:
        ...
        print(f"[mosanic] training {self.dataset}: {n_params/1e6:.1f}M params, "
              f"{n_train} train / {n_val} val cells, {n_genes} target genes "
              f"on {self.device}")
        print(f"[mosanic] epoch     train_loss   val_loss     val_R2    val_r    time")
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % log_every == 0 or is_last:
                print(f"[mosanic] {epoch+1:5d}/{num_epochs:<4d} "
            ...
            if self.early_stopping(val_metrics["r2_mean"]):
                print(f"[mosanic] early stopping at epoch {epoch+1} "
