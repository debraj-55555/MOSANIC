"""
mosanic/evaluation/evaluator.py

Evaluate trained MOSANIC model on L2 + L3 CCC metrics:
...
Usage:
    python -m mosanic.evaluation.evaluator \\
        --config mosanic/configs/breast_config.yaml \\
        --dataset breast_new \\
        --device cuda
"""
...
def evaluate_model(cfg, dataset, device="cuda", channel="all"):
    """
    Run full CCC evaluation on a trained MOSANIC model.
    ...
    if spatial_attn is not None:
        from mosanic.evaluation.metrics import compute_des
    ...
    from mosanic.evaluation.metrics import fetch_omnipath_lr_pairs, compute_cross_db_recall
    ...
    if cell_types is not None:
        from mosanic.evaluation.metrics import compute_niche_clustering
