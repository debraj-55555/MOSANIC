"""
mosaic/cli.py — Unified command-line entry point for MOSAIC.

Usage:
    mosaic preprocess --config configs/breast.yaml [--force]
    mosaic train      --config configs/breast.yaml [--device cuda] [--checkpoint path]
    mosaic evaluate   --config configs/breast.yaml [--device cuda]
    mosaic run        --config configs/breast.yaml [--device cuda]   # full pipeline
...
def _resolve_paths(cfg: dict, dataset: str):
    ...
    ckpt_dir = Path(
        cfg.get("training", {}).get("checkpoint_dir", "mosaic/checkpoints")
    ) / dataset
...
def _load_graph(graph_path: Path):
    ...
    if not graph_path.exists():
        log.error(
            "Graph not found: %s\n"
            "Run: mosaic preprocess --config <config.yaml> first.",
            graph_path,
        )
        sys.exit(1)
...
def cmd_preprocess(args):
    ...
    log.info("=== MOSAIC preprocess: %s ===", dataset)
...
def cmd_train(args):
    """Train MOSAIC on a preprocessed graph."""
    cfg = _load_config(args.config)
    dataset = cfg["dataset"]
    ...
    log.info("=== MOSAIC train: %s on %s for %d epochs ===", dataset, device, n_epochs)
    history = trainer.train(n_epochs)

    log.info("Training complete.")
    log.info("Best val R² = %.4f", max(history["val_r2"]))

    # Load best checkpoint and return MOSAICResult
    from mosaic.api import MOSAICResult
    ...
    log.info("Returning MOSAICResult — use result.lr_pairs(), result.plot_spatial(), etc.")
    return result
...
def cmd_evaluate(args):
    """Evaluate a trained MOSAIC model on CCC metrics."""
    cfg = _load_config(args.config)
    dataset = cfg["dataset"]

    from mosaic.evaluation import evaluate_model

    log.info("=== MOSAIC evaluate: %s ===", dataset)
...
def cmd_run(args):
    """Execute the full MOSAIC pipeline: preprocess -> train -> evaluate."""
    cfg = _load_config(args.config)
    dataset = cfg["dataset"]
    log.info("=== MOSAIC full pipeline: %s ===", dataset)
    ...
    # Step 2: train (returns MOSAICResult)
    ...
    log.info("=== MOSAIC pipeline complete for %s ===", dataset)
    return result
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  mosaic preprocess --config configs/breast.yaml\n"
            "  mosaic train      --config configs/breast.yaml --device cuda\n"
            "  mosaic evaluate   --config configs/breast.yaml --device cuda\n"
            "  mosaic run        --config configs/breast.yaml --device cuda\n"
        ),
    )
    ...
    tr = subparsers.add_parser(
        "train",
        help="Train MOSAIC on a preprocessed heterogeneous graph",
        description=(
            "Load hetero_ccc_graph.pt, build the MOSAIC transformer, "
    ...
    ev = subparsers.add_parser(
        "evaluate",
        help="Evaluate a trained MOSAIC model on CCC metrics",
        description=(
