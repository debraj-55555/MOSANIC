"""
mosanic/analysis/relay.py

Relay network analysis from MOSANIC per-edge attention.
...
Usage:
    python -m mosanic.analysis.relay --dataset breast_new --device cuda:0
"""
...
def load_model_and_data(dataset: str, device: str, config_dir: str = None, data_dir: str = None):
    """Load trained MOSANIC model + hetero graph."""
    from mosanic.models import build_model

    if config_dir is None:
        config_dir = "mosanic/configs"
    if data_dir is None:
        data_dir = "mosanic/data/processed"
    ...
    ckpt_path = Path(cfg.get("training", {}).get("checkpoint_dir", "mosanic/checkpoints")) / dataset / "model_best.pt"
...
def detect_relays(model, data, cell_types, ct_mapping, coords=None,
    ...
    """
    High-level API: detect relay networks from a trained MOSANIC model.
...
def main():
    ...
    data_dir = args.data_dir or "mosanic/data/processed"
